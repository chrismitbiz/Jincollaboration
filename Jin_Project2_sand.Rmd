...
## Phylofactor
```{r, phylfactor1, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.cap= "Relative abundances which significantly changed over time and under elevated CO2 of taxa bins identifed with phylofactor (model: days+CO2, n = 24)."}
require(phylofactor)
...
## Phylofactor
```{r, phylfactor2, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.cap= "Relative abundances which significantly changed over time and under elevated CO2 of taxa bins identifed with phylofactor (model days+CO2, n = 24)."}
require(phylofactor)
...
bin26  <- bin26 %>% left_join(taxtable) %>%  dplyr::select(-OTU_ID, - Species) %>% unique
bin26  <- bin26 %>% mutate(Bin = "Bin26") %>% dplyr::select(Bin, everything())

# sig. factors for CO2 1*, 3. , 6*, 11*, 12***, 14*, 17*, 21., 22*, 24***, 25**

bin <- rbind(bin3, bin6, bin11, bin12,bin14, bin17,bin22, bin24, bin25 , bin26)
kable(bin, booktabs=TRUE, caption = "Taxa represented in bins") %>% kable_styling(latex_options="scale_down")
