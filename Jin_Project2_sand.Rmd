...
```{r vertsigdiff, echo=FALSE, fig.width=10, message=FALSE, warning=FALSE, fig.cap = "some selected features"}
#Soil1 data
physeqBSoil1 <-  prune_samples(sample_data(physeqB.flt)$Soil == "Vertisol", physeqB.flt)
...
#barplot
barplot.rel   <- phyloseq::transform_sample_counts(physeqBSoil1, function(x) x / sum(x) )
subs <- subset_taxa(barplot.rel, Genus %in% c("D_5__Bacillus", "D_5__Paenibacillus"))
subs <- psmelt(subs)
...
```{r chromosigdiff, echo=FALSE, fig.width=10, message=FALSE, warning=FALSE, cache=TRUE, fig.cap = "some selected features"}
