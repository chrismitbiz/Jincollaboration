...
```{r phyloseq, warning=FALSE, message=FALSE, echo=FALSE}
...
```{r inspec, warning=FALSE, message=FALSE, echo=FALSE}
# all features
...
ggbarplot(x = "Var2", y = "value",xlab = "Filtered")+ 
    geom_hline(yintercept=median(rowSums(otu_table(physeqB.flt))), color="red")
dat2
...
subs <- psmelt(subs)
barplot.relsoil1 <- subs   %>% ggbarplot(data = ., x = "Genus", y = "Abundance", add = "mean_se", facet.by = c("Soils"), fill = "CO2", width = 0.5, xlab = "", position = position_dodge(0.7))
barplot.relsoil1 
