...
## Phylofactor
```{r, phylfactor1, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.cap= "Change of relative abundances which significantly changed over time and under elevated CO2 of taxa bins identifed with phylofactor (n = 24)."}
require(phylofactor)
...
#factor 2, 4 7,8, 14, 16, 17, 21, 22, 23, 24, 27, 29, 30, 31, 35, 39, 42, 43, 48, 49
...
bin.projection <- pf.BINprojection(PF.mult,factor=15, rel.abund=T)  
...
colori4<-c("red","green","blue", "purple", "lightblue")
...
legend("left", legend=binnm, fill=smoothColors(colori4),cex=0.5 )
```
...
kable(bin, booktabs=TRUE, caption = "Taxa of bin 4") %>% kable_styling(latex_options="scale_down")
