...
SVs      <-   qiime2R::read_qza("~/Documents/Study/LaTrobe/Research/phD/Jin_Project2_Sand/Jincollaboration/qiime2files/feature_table_insertiontreefiltered.qza")

taxonomy <- read_tsv("~/Documents/Study/LaTrobe/Research/phD/Jin_Project2_Sand/Jincollaboration/qiime2files/taxonomy_silva.tsv") %>% filter(Taxon != "categorical")  %>% rename(Feature.ID = `Feature ID`)

taxtable <- taxonomy %>% as_tibble() %>% separate(Taxon, sep=";", c("Kingdom","Phylum","Class","Order","Family","Genus","Species")) 

tree <- read_qza("~/Documents/Study/LaTrobe/Research/phD/Jin_Project2_Sand/Jincollaboration/qiime2files/insertion-tree.qza")
...
## Comparison by CO2
```{r barCO2, warning=FALSE, message=FALSE, echo=FALSE,cache=TRUE}
...
```{r barCO2_class, echo=FALSE, fig.width=10, message=FALSE, warning=FALSE, cache=TRUE}
#CLASS LEVEL
#Soil1
#use relative abundances and glomerate to x level
...
```{r bardays, echo=FALSE, fig.width=10, message=FALSE, warning=FALSE, cache=TRUE}
#Phylum level
#use relative abundances and glomerate to x level
physeqB.flt.rel.glom <- tax_glom(physeqB.flt, "Phylum")
physeqB.flt.rel.glom <- phyloseq::transform_sample_counts(physeqB.flt.rel.glom, function(x) x / sum(x) )
...
## Vertisol - Sign. diff. on genus level (Wilcoxon test *p* < 0.05 holm corrected)
```{r vertsigdiff, echo=FALSE, fig.width=10, message=FALSE, warning=FALSE}
#Soil1 data
physeqBSoil1 <-  prune_samples(sample_data(physeqB.flt)$Soil == "Vertisol", physeqB.flt)
#physeqBSoil1 = subset_taxa(physeqBSoil1, !is.na(Genus) & !Genus %in% c("", "uncharacterized"))
...
wilcoxotest <-  barplot.rel.melt %>% compare_means(Abundance ~ CO2, data = ., method = "wilcox.test", paired = FALSE,
  group.by = "Genus", ref.group = NULL, symnum.args = list(), p.adjust.method = "holm") %>% as.data.frame() %>% filter(p.adj <= 0.05)
kable(wilcoxotest %>% filter(Genus != "") %>% filter(Genus != "D_5__metagenome"),booktabs=TRUE) %>% 
  kable_styling(latex_options="scale_down")

#barplot
barplot.rel   <- phyloseq::transform_sample_counts(physeqBSoil1, function(x) x / sum(x) )
subs <- subset_taxa(barplot.rel, Genus %in% c("D_5__Bacillus", "D_5__Paenibacillus","D_5__Agromyces", "D_5__Pseudonocardia", "D_5__Aeromicrobium"))
subs <- psmelt(subs)
...
## Chromosol - Sign. diff. on genus level (Wilcoxon test *p* < 0.05 holm corrected)
```{r chromosigdiff, echo=FALSE, fig.width=10, message=FALSE, warning=FALSE, cache=TRUE}
...
#barplot
barplot.rel   <- phyloseq::transform_sample_counts(physeqBSoil2, function(x) x / sum(x) )
subs <- subset_taxa(barplot.rel, Genus %in% c("D_5__Bacillus", "D_5__Burkholderia-Caballeronia-Paraburkholderia"))
subs <- psmelt(subs)
...
## by soil
```{r alphasoil, warning=FALSE, message=FALSE, echo=FALSE, cache=TRUE}
