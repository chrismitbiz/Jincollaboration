...
bin13  <- bin13 %>% left_join(taxtable) %>%  dplyr::select(-OTU_ID, - Species) %>% unique
...
bin18  <- bin18 %>% left_join(taxtable) %>%  dplyr::select(-OTU_ID, - Species) %>% unique
...
bin23  <- bin23 %>% left_join(taxtable) %>%  dplyr::select(-OTU_ID, - Species) %>% unique
...
bin <- rbind(bin4, bin7, bin12, bin13,bin14, bin18,bin23, bin24, bin25 , bin26)
kable(bin, booktabs=TRUE, caption = "Taxa represented in bins") %>% kable_styling(latex_options="scale_down")
